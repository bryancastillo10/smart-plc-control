...
function loadConfig() {
  try {
    return { ...DEFAULTS, ...JSON.parse(readFileSync(resolve(ROOT, CONFIG), 'utf8')) }
  } catch {
    return DEFAULTS
  }
}
...
export function measure(root = ROOT, config = loadConfig()) {
  ...
  const cwd = { cwd: root }
  const coverage = JSON.parse(readFileSync(resolve(root, config.coverage), 'utf8'))
  ...
  for (const file of files) {
    ...
    measured[best.nodeId].loc += readFileSync(resolve(root, file), 'utf8').split('\n').length
  }
